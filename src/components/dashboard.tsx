    ...
    <>
      ...
      {statusBar && (
        <div className="px-5 w-full max-w-lg mx-auto pt-1 pb-1">
          <div className="flex items-center justify-between rounded-[8px] bg-bg-secondary px-4 py-3">
            <div className="flex items-center gap-2.5">
              <div className={`w-2 h-2 rounded-full shrink-0 ${statusBar.next.action ? "bg-[#2563EB]" : "bg-text-muted/30"}`} />
              <span className="text-[15px] font-semibold text-text-primary">{statusBar.now.label}</span>
              <svg className="w-3.5 h-3.5 text-text-muted/40 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5" />
              </svg>
              <span className={`text-[15px] font-semibold ${statusBar.next.action ? "text-[#2563EB]" : "text-text-muted"}`}>{statusBar.next.label}</span>
            </div>
            {statusBar.next.time && (
              <span className="text-[13px] font-bold text-text-muted">{statusBar.next.time}</span>
            )}
          </div>
        </div>
      )}
